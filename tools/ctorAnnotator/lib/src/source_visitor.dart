...
class SourceVisitor implements AstVisitor {
  List<String> ctorLocationStrings = [];
  ...
  visitArgumentList(ArgumentList node) {
    ...
    var patchedCtor = ctorLocationStrings.length > 0;
    String tokenLocation;
    if (patchedCtor) tokenLocation = ctorLocationStrings.removeAt(0);

    new ArgumentListVisitor(this, node).visit(patchedCtorString: tokenLocation);
  }
  ...
  visitInstanceCreationExpression(InstanceCreationExpression nn) {
    ...
    if (WIDGET_CLASS_NAMES.contains(node.constructorName)) {
      ctorLocationStrings.add("${nn.offset}");
    }
  ...
  /// specified functions.
  void visit(AstNode node, {void before(), void after(), patchedCtor: false}) {
    if (node == null) return;
